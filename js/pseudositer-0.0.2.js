...
( function($) {
	...
	settings = {
	    ...
	    load = function(url) { // The abstract load function.
		var urlSplit = url.split('/'),
		level,
		urlToThisLevel,
	        $directory;
		console.log(urlSplit);
		for(level = 0; level < urlSplit.length; level++) {
		    urlToThisLevel = urlSplit.slice(0, level).join('/');
	    
		    // Load a directory.
		    if(level < urlSplit.length)  {
		
			// Use an existing element if possible.
			$directory = $('.' + settings.directoryClass + '.' + settings.levelClass + level);
			...
			$directory.load(settings.hiddenUrl + urlToThisLevel + ' ' + settings.linkSelector, function(responseText, textStatus) {
				...
				$.each($(this).children('a'), function() {
					...
					name = settings.preprocessName($link.attr('href')); // Get the name from the link, because the text is often truncated.
					$link.text(name);
		...
		methods = {
		    ...
		    'empty': function(event, callback, args) {
		    ...
		    'hide' : function(event, callback, args) {
			return $(this).slideUp('fast', function() {
				if(callback) {
				    callback.call($(this), args);
				}
		    ...
		    'show' : function(event, callback, args) {
			...
			return this.each(function() {
				var $link = $(this),
				settings = $link.data('settings'),
				$directory = $('.' + settings.directoryClass + '.' + settings.levelClass + (settings.level)),
				hash = window.location.hash.split('/')[settings.level];
				if(hash) {
