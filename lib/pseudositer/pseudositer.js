  ...
  */
  ...
  (function($) {
    /*
      ...
      #
    */
    var clipExtension, contentClass, createIndex, destroyIndex, download, errorClass, events, getClassForPath, getContentContainer, getCurrentFragment, getCurrentPath, getIndexClassForLevel, getIndexContainer, getIndexLevel, getIndexTrail, getPathDepth, getSuffix, hideContent, hideError, hideLoadingNotice, indexClass, indexContainerClass, isPathToFile, linkClass, loadHtml, loadImage, loadText, loadingClass, log, selectLink, selectedLinkClass, showContent, showError, showLoadingNotice, staleClass;
    events = ['startUpdate', 'doneUpdate', 'startLoading', 'failedLoading', 'doneLoading', 'alwaysLoading', 'destroyIndex', 'createIndex', 'selectLink', 'loadImage', 'loadText', 'loadHtml', 'loadDefault', 'hideContent', 'showContent', 'showError', 'destroy'];
    /*
      #
    ...
    */
    ...
    createIndex = function(evt, dfd, path, $links) {
      ...
      showIndex = new $.Deferred().done(function() {
        return $index.fadeIn('slow', function() {
          return dfd.resolve();
        });
      });
      if (path === prevPath) {
    ...
    selectLink = function(evt, dfd, path) {
      var $index, level;
      level = path.split('/').length - 2;
      $index = $("." + (getIndexClassForLevel(level)));
      log("." + (getIndexClassForLevel(level)));
      log(path);
    ...
    loadImage = function(evt, dfd, pathToImage) {
      var $img, $tmp;
      if (this.logging) log("loadImage( " + dfd + ", " + pathToImage + " )");
      $tmp = $('<div />').css({
        height: '0px',
        width: '0px'
      }).appendTo($('body'));
    ...
    loadText = function(evt, dfd, pathToText) {
      if (this.logging) log("loadText( " + dfd + ", " + pathToText + " )");
    ...
    loadHtml = function(evt, dfd, pathToHtml) {
      if (this.logging) log("loadHtml( " + dfd + ", " + pathToHtml + " )");
    ...
    download = function(evt, dfd, pathToFile) {
      if (this.logging) log("download( " + dfd + ", " + pathToFile + " )");
      ...
      */
      ...
      showIndices = function(path) {
        ...
        $.each(trails, function(idx, trail) {
          ...
          indexCreated.done(function() {
            return trigger('selectLink', linkSelected, trail);
          });
          return promises.push(linkSelected.promise());
        });
